...
export function checkPlayerWin (x: string, state: Cells, game: Game, clients: ClientsMap) {
    ...
    if( state.c1 == x && state.c2 == x && state.c3 == x) setWinner(x, ["c1", "c2", "c3"]);
    if( state.c4 == x && state.c5 == x && state.c6 == x) setWinner(x, ["c4", "c5", "c6"]);
    if( state.c7 == x && state.c8 == x && state.c9 == x) setWinner(x, ["c7", "c8", "c9"]);
    if( state.c1 == x && state.c4 == x && state.c7 == x) setWinner(x, ["c1", "c4", "c7"]);
    if( state.c2 == x && state.c5 == x && state.c8 == x) setWinner(x, ["c2", "c5", "c8"]);
    if( state.c3 == x && state.c6 == x && state.c9 == x) setWinner(x, ["c3", "c6", "c9"]);
    if( state.c1 == x && state.c5 == x && state.c9 == x) setWinner(x, ["c1", "c5", "c9"]);
    if( state.c3 == x && state.c5 == x && state.c7 == x) setWinner(x, ["c3", "c5", "c7"]);
...
export function endGame (game: Game, clients: ClientsMap, winner?: Winner) {
    ...
    const payLoad = {
        "method": "end",
        "message": winner === null ? 'match draw' : `Player ${winner?.symbol} won`,
        "gameId": game.gameId,
